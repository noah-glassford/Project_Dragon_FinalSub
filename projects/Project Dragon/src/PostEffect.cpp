...
void PostEffect::Init(unsigned width, unsigned height)
{
	...
	_shaders[_shaders.size() - 1]->LoadShaderPartFromFile("shader/passthrough_vert.glsl", GL_VERTEX_SHADER);
	_shaders[_shaders.size() - 1]->LoadShaderPartFromFile("shader/passthrough_frag.glsl", GL_FRAGMENT_SHADER); 
...
Framebuffer* PostEffect::GetBuffer(int index)
{
