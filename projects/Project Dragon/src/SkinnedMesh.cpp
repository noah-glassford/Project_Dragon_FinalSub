...
bool GLTFSkinnedMesh::LoadFromFile(std::string const path)
{
	...
	for (int i = 0; i < anim.channels.size(); i++)
	{
		...
		tinygltf::AnimationSampler& sampler = anim.samplers[anim.channels[i].sampler];
		int keyframesIndex = sampler.input;
		
		KeyFrames temp;
		temp._input = RetrieveModelData<float>(keyframesIndex, TINYGLTF_TYPE_SCALAR);
		
		if (anim.channels[i].target_path == "rotation")
		temp._output = RetrieveModelData<KF_Output>(sampler.output, TINYGLTF_TYPE_VEC4);
		else
		{
			std::vector<glm::vec3> temp_2;
			temp_2 = RetrieveModelData<glm::vec3>(sampler.output, TINYGLTF_TYPE_VEC3);
			for (int i = 0; i < temp_2.size(); i++)
			{
				KF_Output output;
				output.v3 = temp_2[i];
				temp._output.push_back(output);
			}

		}
		m_StoredAnimation.push_back(temp);

	}

	m_animationLength = GetAnimationLength(anim);
...
typedef glm::vec<4, uint8_t, glm::highp> UByteVec4;
...
bool GLTFSkinnedMesh::LoadMesh(int const meshIndex, int const associatedSkin)
{
	...
	for (int i = 0; i < mesh.primitives.size(); i++)
	{
		...
		std::vector<UByteVec4> jointsRaw =
			RetrieveModelData<UByteVec4>(primitive.attributes["JOINTS_0"], TINYGLTF_TYPE_VEC4);
...
KF_Output GLTFSkinnedMesh::SampleAnimationChannel(int channelIndex, float time, tinygltf::Animation& const animation, int const accessorType)
{
	tinygltf::AnimationSampler& sampler = animation.samplers[animation.channels[channelIndex].sampler];
	...
	std::vector<float> keyframeTimes =
		m_StoredAnimation[channelIndex]._input;
	//m_StoredAnimation
	...
	KF_Output frame1 = m_StoredAnimation[channelIndex]._output[frame1Index];
	KF_Output frame2 = m_StoredAnimation[channelIndex]._output[frame1Index];

	// Linear interpolation!!
	return frame1 * (1.0f - t) + frame2 * t;
}
...
void GLTFSkinnedMesh::Draw(Shader::sptr& const shader, glm::mat4& const viewProjection, glm::mat4& const worldTransform)
{
	...
	shader->Bind();
	AssetLoader::GetRendererFromStr("hands").Material->Apply();
	...
	//
		if (channel.target_path == "translation")
		{
			glm::vec3 translation =
				SampleAnimationChannel(i, m_animationTime, animation, TINYGLTF_TYPE_VEC3).v3;
			transform->m_position = translation;
		}
		else if (channel.target_path == "rotation")
		{
			glm::vec4 rotation =
				SampleAnimationChannel(i, m_animationTime, animation, TINYGLTF_TYPE_VEC4).v4;
