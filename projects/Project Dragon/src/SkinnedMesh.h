...
union KF_Output
{
    ...
    KF_Output& operator*(const KF_Output& other)
    {
        v4 *= v4;
        return *this;
    }
    KF_Output& operator*(const float& other)
    {
        v4 *= other;
        return *this;
    }
...
public:
  ...
  //  template<typename T>
    KF_Output SampleAnimationChannel(int channelIndex, float time,
    ...
    std::vector<KeyFrames> m_StoredAnimation;
